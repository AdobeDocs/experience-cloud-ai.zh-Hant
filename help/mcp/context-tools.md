---
title: CX Enterprise MCP中的工作階段內容工具
description: 瞭解為所有CX Enterprise MCP工具呼叫設定組織、沙箱和資料檢視內容的核心工具。
...
# Adobe CX Enterprise MCP中的工作階段內容工具 {#mcp-core}

Adobe CX Enterprise MCP包含一組工作階段內容工具，可建立Adobe組織、Adobe Experience Platform沙箱和Customer Journey Analytics資料檢視，以供所有其他產品工具在其中運作。 不需要額外的授權或啟用 — 這些工具在連線至[CX Enterprise MCP伺服器](overview.md)後，可供所有已驗證的使用者使用。

## 上下文的運作方式 {#mcp-core-how}

CX Enterprise MCP將每個工具呼叫的範圍限定為一個作用中的Adobe組織。 除此之外，內容需求取決於產品：
...
- [安裝Adobe CX Enterprise MCP](install.md) — 如何連線您的MCP使用者端，包括產品內容設定區段。
- [存取CX Enterprise MCP工具](access.md) — 存取需求依產品而定。